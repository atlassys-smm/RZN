...
```mermaid
flowchart LR
    A["Гость<br/>(guest_id, 24ч)"] -->|"OAuth в течение 24ч"| B["Авторизован<br/>(OAuth-поля)"]
    A -->|"24 часа без авторизации"| C["Обезличен<br/>(anonymized)"]
    B -->|"Действие администратора"| D["Заблокирован<br/>(deactivated)"]
    C -->|"Действие администратора"| D
```

Все состояния — статус `active`, кроме `deactivated`. Гость и обезличенный определяются по полям `guest_id` и `guest_expires_at`.

**Гость** — создаётся при первом сканировании QR-кода, имеет `guest_id` и `guest_expires_at` (24 часа), может пройти активности. Результаты привязаны к `id` записи.

**Авторизованный** — гость, прошедший OAuth (Яндекс ID или VK ID) в течение 24 часов. `guest_expires_at` обнуляется. Получает доступ к цифровому профилю, истории активностей, рекомендациям.

**Обезличенный** — гость, не авторизовавшийся в течение 24 часов (`guest_expires_at` истёк). Результаты и аналитика сохраняются, но связь с пользователем утеряна навсегда.

**Заблокированный** — пользователь, заблокированный администратором (`status = deactivated`). Вход невозможен, данные сохранены.
...
flowchart TD
    ...
    Choice -->|Гость| CreateGuest["Создание User<br/>(status=active, guest_id, guest_expires_at)"]
