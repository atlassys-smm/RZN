...
- Гость сканирует QR-код на терминале и выбирает «Продолжить как гость»
- Сервер генерирует `guest_id` и сохраняет его в браузере/локальном хранилище телефона
- При сканировании QR на другом терминале браузер отправляет `guest_id` — система узнаёт того же гостя
- Все результаты гостя привязываются к `guest_id`
- **Привязка к профилю:** если в рамках гостевой сессии пользователь авторизуется через OAuth, система связывает `guest_id` с `user_id` и все результаты закрепляются за пользователем
